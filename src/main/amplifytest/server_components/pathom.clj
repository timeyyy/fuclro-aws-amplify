(ns amplifytest.server-components.pathom
  (:require
    ...
    ;; Central registry
    [amplifytest.server-components.pathom-wrappers :refer [pathom-registry]]
    [amplifytest.server-components.config :refer [config]]

    ;; ALL namespaces that use pathom-wrappers MUST be included for auto-registration to work
    amplifytest.model.user))
